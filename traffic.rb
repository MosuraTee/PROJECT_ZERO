...
class Main
  ...
  @@incidents = [
    ...
    {street: "man", time: "08 AM", time_until: "10 AM", condition: "BAC_test"}
  ]
  ...
  def self.display_table
    rows = []
    @@incidents.each do |i|
      rows << [i[:street, :time, :time_until, :condition]
    end
    table = Terminal::Table.new :headings => ['Street', 'Time Begin',
      'Time Finish', 'Condition'], :rows => rows
    puts table
  end
